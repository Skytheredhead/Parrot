...
initialization provenance, and reviewed canonical schema digest, availability of 78 private authority tables, 90 aggregate-only
...
provider checks are `NotConfigured`, current module code and outbox lease shape are `NotVerified`, and audit continuity is only

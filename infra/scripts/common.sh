...
DEPLOYMENT_ENV_KEYS=(
  ...
  BACKUP_DIR BACKUP_MOUNT_OWNER_UID GATEWAY_IMAGE WORKER_IMAGE GATEWAY_LOOPBACK_PORT EDGE_IMAGE EDGE_LOOPBACK_PORT
  EDGE_SERVER_NAME EDGE_CONFIG_TEMPLATE_PATH CLAMAV_IMAGE CLAMAV_CONFIG_PATH
...
require_base_identity() {
  ...
  [[ "${BACKUP_MOUNT_OWNER_UID:-}" =~ ^[0-9]{1,10}$ ]] \
    || die "backup mount owner UID must be an explicit numeric UID"
...
assert_trusted_directory() {
  local directory="$1" label="$2" private="${3:-false}" trusted_owner="${4:-}" mode owner logical physical
  ...
  [[ "$owner" == "$(id -u)" || "$owner" == 0 || ( -n "$trusted_owner" && "$owner" == "$trusted_owner" ) ]] \
    || die "$label must be owned by the runtime operator, root, or its explicit trusted owner: $directory"
}
...
assert_environment_path_chain() {
  local service_root="${1:-/srv}" environment="${2:-$DEPLOY_ENVIRONMENT}" trusted_root_owner=""
  [[ "$environment" == production || "$environment" == staging ]] || die "invalid environment path-chain identity"
  [[ "$service_root" != /mnt/bigboi ]] || trusted_root_owner="$BACKUP_MOUNT_OWNER_UID"
  assert_trusted_directory "$service_root" "service root" false "$trusted_root_owner"
